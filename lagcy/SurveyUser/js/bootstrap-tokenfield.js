...
(function (factory) {
    ...
    var Tokenfield = function (element, options) {
        ...
        $.each(this._delimiters, function (index, character) {
            ...
            if (pos >= 0)
                _self._delimiters[index] = '\\' + character;
        });
        ...
        // Create a wrapper
        this.$wrapper = $('<div class="tokenfield form-control shareInput" />')
    ...
    Tokenfield.prototype = {
        ...
        , createToken: function (attrs, triggerChange) {
            ...
            if (!attrs.value.length || !attrs.label.length || attrs.label.length <= this.options.minLength)
                return
            ...
            $token
                    .on('mousedown', function (e) {
                        if (_self._disabled || _self._readonly)
                            return false
        ...
        , setTokens: function (tokens, add, triggerChange) {
            ...
            if (!add)
                this.$wrapper.find('.token').remove()
        ...
        , keydown: function (e) {
            ...
            switch (e.keyCode) {
                ...
                case 65: // a (to handle ctrl + a)
                    if (this.$input.val().length > 0 || !(e.ctrlKey || e.metaKey))
                        break
                    this.activateAll()
                    e.preventDefault()
                    break
                ...
                case 13: // enter
                    ...
                    // Edit token
                    if (e.keyCode === 13) {
                        if (!this.$copyHelper.is(document.activeElement) || this.$wrapper.find('.token.active').length !== 1)
                            break
                        if (!_self.options.allowEditing)
                            break
        ...
        , keyup: function (e) {
            ...
            switch (e.keyCode) {
                case 8: // backspace
                    if (this.$input.is(document.activeElement)) {
                        if (this.$input.val().length || this.lastInputValue.length && this.lastKeyDown === 8)
                            break
        ...
        , remove: function (e, direction) {
            ...
            if (!this.$wrapper.find('.token').length || e.type === 'click' || firstToken)
                this.$input.focus()
        ...
        , focusInput: function (e) {
            if ($(e.target).closest('.token').length || $(e.target).closest('.token-input').length || $(e.target).closest('.tt-dropdown-menu').length)
                return
