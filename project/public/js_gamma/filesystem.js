...
var MiddleBlock_FS = function(controller){
	...
	var collectionBlock =	new FileFolderBlock(new Folder(), this);
	...
	(function(){
		...
		folderBlock.setTitle('工作区');
		module.find('.content:first').append(folderBlock.getModule());
		
		
		collectionBlock.setTitle('集合');
		module.find('.content:first').append(collectionBlock.getModule());
	...
	this.openFolder = function(id){
		getAllInFolder(id)
			.then(function(result){
				...
				collectionBlock.addList(result.collection, viewType);
...
const	Panel_MENU_SET = [
	...
	},
	{
		...
		authority: 1,
		action: function(){
			singleLineInput('新建工作区', '请输入工作区名称', function(input, modal){
				modal.modal('hide');
			});
		}
	},
